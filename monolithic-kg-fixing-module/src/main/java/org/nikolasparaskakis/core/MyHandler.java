...
public class MyHandler implements AutoCloseable {
     ...
     */
    public MyHandler(Set<OWLAxiom> ontologyAxioms, IRI iri, int reasonerSelection) {
    ...
    public MyHandler(Set<OWLAxiom> ontologyAxioms, int reasonerSelection) {
        // Create ontology from the provided axioms
        OWLOntologyManager manager;
        try {
            manager = createOWLOntologyManager();
...
//            reasonerFactory = new FaCTPlusPlusReasonerFactory();
    ...
    public ConsistencyCheckOutcome isConsistentSafe(long timeoutMillis) {
        ...
        // (Optional) stricter config with a progress monitor you can log
        OWLReasonerConfiguration cfg = new SimpleConfiguration(timeoutMillis);
        ...
        Thread t = new Thread(() -> {
            ...
            try {
                ...
                // quick sanity
                int axCount = this.ontology.getAxiomCount();
                if (axCount == 0) {
                    System.err.println("[ConsistencyCheck] Warning: ontology has 0 axioms.");
                }

                r = rf.createReasoner(this.ontology, cfg);
                ref.set(r);
        ...
        try {
            ...
            if (t.isAlive()) {
                ...
                t.interrupt();
                t.join(1500);
                return new ConsistencyCheckOutcome(null,
            ...
            if (failure.get() != null) {
                ...
                System.err.println("[ConsistencyCheck] Reasoner failed: " + ex.getClass().getName() + ": " + ex.getMessage());
                System.err.println(stackTrace(ex));
                return new ConsistencyCheckOutcome(null, ConsistencyCheckOutcome.Status.FAILED, ex);
            }
    ...
    public ExplanationOutcome getExplanationsSafe(long perOpTimeoutMillis, long timeoutMillis, int explanationsLimit) {
        ...
        CustomExplanationProgressMonitor<OWLAxiom> monitor =
                new CustomExplanationProgressMonitor<>(durationTimeoutMillis, explanationsLimit);
