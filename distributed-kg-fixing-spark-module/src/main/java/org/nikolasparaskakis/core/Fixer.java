...
//        NamedNulls++;
    ...
    public ConsistencyCheckOutcome CheckConsistency(long timeoutMillis) {
        ...
        try (ModuleHandler moduleHandler = new ModuleHandler(ontology.getAxioms(), reasonerSelection)) {
            consistencyCheckOutcome = moduleHandler.isConsistent(timeoutMillis);
        }
    ...
    public ConsistencyCheckOutcome CheckConsistency() {
        ...
        try (ModuleHandler moduleHandler = new ModuleHandler(ontology.getAxioms(), reasonerSelection)) {
            result = moduleHandler.isConsistent();
        }
        return result;
    }
    ...
    public int countInconsistencies() {
        ...
        int result = 0;
        try (ModuleHandler moduleHandler = new ModuleHandler(ontology.getAxioms(), reasonerSelection)) {
            if (!moduleHandler.isConsistent().getConsistent()) {
                // Some inconsistency should be found TODO: CHECK THIS and REFACTOR
                ExplanationOutcome j = moduleHandler.getExplanations(perOpTimeoutMillis, reasonerTimeoutMillis, explanationsLimit);
